...
MenuState::MenuState(TextureManager* game_textures) : GameState(game_textures)
{
	...
	Background* background = new Background("menu_background");
...
{
	if (!textur_map->loadTexture("gfx/menu_background.png", "menu_background"))
	{
		return false;
	}
	textures_id.push_back("menu_background");

	if (!textur_map->loadTexture("gfx/menu_buttons.png", "menu_buttons"))
	{
		return false;
	}
	textures_id.push_back("menu_buttons");

	if (!textur_map->loadTexture("gfx/smoke.png", "smoke"))
	{
		return false;
	}
