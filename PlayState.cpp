...
PlayState::PlayState(TextureManager* game_textures, GLBL::signal signal) : GameState (game_textures)
{
	...
	Background* background = new Background("play_background");
...
{
	if (!textur_map->loadTexture("gfx/play_background.png", "play_background"))
	{
		return false;
	}
	textures_id.push_back("play_background");

	if (!textur_map->loadTexture("gfx/play_buttons.png", "play_buttons"))
	{
		return false;
	}
	textures_id.push_back("play_buttons");

	if (!textur_map->loadTexture("gfx/deck.png", "deck"))
	{
		return false;
	}
	textures_id.push_back("deck");

	if (!textur_map->loadTexture("gfx/play_hud.png", "play_hud"))
	{
		return false;
	}
	textures_id.push_back("play_hud");

	if (!textur_map->loadTexture("gfx/player_icons.png", "player_icons"))
	{
		return false;
	}
	textures_id.push_back("player_icons");

	if (!textur_map->loadTexture("gfx/overlay.png", "overlay"))
	{
		return false;
	}
	textures_id.push_back("overlay");

	if (!textur_map->loadFont("fonts/calibri.ttf", "arial", 48))
	{
		return false;
	}
...
bool PlayState::loadDeck()
{
	...
	std::ifstream fin("cnfg/deck_cnfg.txt"); 
	if (!fin.is_open())
	{
	...
	for (int row = 1, line = 0, cur_card = 1; cur_card <= GLBL::TOTAL_CARDS; cur_card++)
	{
		...
		{
			int card_type, cost, power;
			GLBL::parameters_type cost_type, power_type;
			if (sscanf(str.c_str(), "%d %d %d %d %d", &card_type, &cost_type, &cost, &power_type, &power) != 5)
			{
				break;
			}
...
bool PlayState::loadSave()
{
	...
	std::ifstream fin("save/save_game.txt");
	if (!fin.is_open())
	{
...
bool PlayState::load_player(std::ifstream& fin, std::string& str)
{
	...
	if (std::getline(fin, str))
	{
		int player_type, hp, mp, gold;
		GLBL::player_side player_side;
		if (sscanf(str.c_str(), "%d %d %d %d %d", &player_type, &player_side, &hp, &mp, &gold) != 5)
			return false;
...
bool PlayState::load_player_cards(std::ifstream& fin, std::string & str)
{
	...
	for (int i = 0; i < 7; i++)
	{
		if (std::getline(fin, str))
		{
			...
			int card_type, cost, power, src_x, src_y, dst_x, dst_y;
			GLBL::parameters_type cost_type, power_type;
			if (sscanf(str.c_str(), "%d %d %d %d %d %d %d %d %d", &card_type, &cost_type, &cost, &power_type, &power, &src_x, &src_y, &dst_x, &dst_y) != 9)
				return false;
...
bool PlayState::load_remaining_cards(std::ifstream& fin, std::string& str)
{
	...
	{
		int card_type, cost, power, src_x, src_y;
		GLBL::parameters_type cost_type, power_type;
		if (sscanf(str.c_str(), "%d %d %d %d %d %d %d", &card_type, &cost_type, &cost, &power_type, &power, &src_x, &src_y) != 7)
			return false;
